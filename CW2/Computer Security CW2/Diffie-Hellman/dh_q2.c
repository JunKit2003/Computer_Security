...
int main() {
    ...
    // Compute public keys
    BN_mod_exp(ya, g, xa, p, ctx); // y_a = g^x_a mod p
    BN_mod_exp(yb, g, xb, p, ctx); // y_b = g^x_b mod p

    // Compute shared keys
    BN_mod_exp(kab1, yb, xa, p, ctx); // K_ab = y_b^x_a mod p
    BN_mod_exp(kab2, ya, xb, p, ctx); // K_ab (from other side)

    // Output
    char *ya_str = BN_bn2dec(ya);
    char *yb_str = BN_bn2dec(yb);
    char *kab1_str = BN_bn2dec(kab1);
